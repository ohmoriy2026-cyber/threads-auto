    ...
    if not api["threads"]: st.info("API設定を完了してください。")
        ...
        with st.spinner("集計中..."):
            ...
            if raw:
                ...
                c1.metric("📝 累計投稿", f"{len(df)} 件")
                c2.metric("👀 累計閲覧", f"{df['views'].sum():,}")
                c3.metric("❤️ 累計いいね", f"{df['likes'].sum():,}")
                c4.metric("💬 累計返信", f"{df['replies'].sum():,}")
                st.divider()
                st.subheader("📅 本日の予約状況")
                sheet_data = get_sheet_data(api["sheet_id"], api["g_json"])
                today_str = datetime.now().strftime('%Y/%m/%d')
                today_list = [r for r in sheet_data if r.get('投稿日') == today_str]
...
# ------------------------------------------
    ...
    if not api["gemini"]: st.warning("API設定を先に済ませてください。")
        ...
        def draw_ui(key):
            c1, c2 = st.columns(2)
            with c1: 
                gender = st.radio("性別", ["女性", "男性", "指定なし"], key=f"g_{key}")
                age = st.multiselect("年代", ["10代", "20代", "30代", "40代", "50代〜"], default=["20代", "30代"], key=f"a_{key}")
            with c2:
                tone = st.selectbox("トーン", tone_list, key=f"t_{key}")
                length = st.slider("文字数", 30, 400, 80, key=f"l_{key}")
            opts = ["手動入力"] + [t["title"] for t in templates]
            sel = st.selectbox("🧠 テンプレート", opts, key=f"tmp_{key}")
            ref = next((t["content"] for t in templates if t["title"] == sel), "") if sel != "手動入力" else ""
            ref_post = st.text_area("🧠 参考投稿", value=ref, key=f"ra_{key}")
            custom = st.text_area("✍️ 自由指示", key=f"cp_{key}")
        ...
        def show_final(key, url, img):
            if key in st.session_state:
                ...
                with st.container(border=True):
                    ...
                    st.text_area("本文編集", key=mk, height=150)
                    st.text_area("リプライ編集", key=rk, height=80)
                    cn, cs = st.columns(2)
                    if cn.button("🚀 即時投稿", key=f"n_{key}"):
                        mid = post_to_threads(api["threads"], st.session_state[mk], image_url=img)
                        if mid: time.sleep(5); post_to_threads(api["threads"], st.session_state[rk], reply_to_id=mid); st.success("成功！")
                    with cs:
                        dv, tv = st.date_input("日", key=f"d_{key}"), st.time_input("時", key=f"t_{key}")
                        if st.button("🗓️ 予約登録", key=f"br_{key}"):
                            row = ["", st.session_state[mk], dv.strftime('%Y/%m/%d'), str(tv.hour), str(tv.minute), "pending", "", "", st.session_state[rk], img]
        ...
        with tab1:
            ...
            genres = {
                ...
                "👠 靴": "558885", "👜 バッグ・小物・ブランド": "216129", "⌚ 腕時計": "558929",
                "💎 ジュエリー・アクセ": "200162", "💄 美容・コスメ・香水": "100939", "💊 ダイエット・健康": "100143",
                "🏥 医薬品・介護": "551169", "🍎 食品": "100227", "🍪 スイーツ・お菓子": "551167",
                "🍹 水・ソフトドリンク": "100316", "🍺 ビール・洋酒": "510915", "🍶 日本酒・焼酎": "510901",
                "🛋 インテリア・収納": "100804", "🍳 キッチン・調理器具": "558944", "🛁 日用品雑貨・文房具": "215783",
                "🔌 家電": "562631", "📸 カメラ・スマホ・TV": "211742", "💻 パソコン・周辺機器": "100026",
                "⚽ スポーツ・アウトドア": "101070", "⛳ ゴルフ用品": "101077", "🚗 車・バイク用品": "503190",
                "🧸 おもちゃ": "101164", "🎨 ホビー": "101165", "🎸 楽器・音響機器": "112493",
                "🎮 ゲーム": "101205", "🐱 ペット用品": "101213", "🍼 ベビー・キッズ": "100533",
                "📚 本・雑誌・コミック": "200376", "📀 CD・DVD": "101240", "💠 その他": "custom"
            }
            sel_g = st.selectbox("ジャンルを選択してください", list(genres.keys()), key="rank_list")
            ...
            if "items" in st.session_state:
                for i, item in enumerate(st.session_state["items"]):
                    ...
                    if c2.button(f"選ぶ: {item['itemName'][:40]}...", key=f"sel_{i}"): st.session_state["active"] = item
                if "active" in st.session_state:
                    st.divider(); t_str, tone, length, ref, custom = draw_ui("t1")
                    if st.button("✨ 生成", key="gen_t1_b"):
                        it = st.session_state["active"]
                        txt = generate_post_text(it["itemName"], it["itemPrice"], t_str, tone, length, custom, ref, api["gemini"])
                        st.session_state["r1"] = {"text": txt}
                    show_final("r1", st.session_state["active"]["itemUrl"], st.session_state["active"]["mediumImageUrls"][0]["imageUrl"])

        with tab2:
            url_in = st.text_input("楽天URL", key="url_in_v4")
            if st.button("取得", key="f_url"): st.session_state["u_info"] = get_item_info_from_url(url_in)
            if "u_info" in st.session_state:
                info = st.session_state["u_info"]
                st.image(info["imageUrl"], width=150); t_str, tone, length, ref, custom = draw_ui("t2")
                if st.button("✨ 生成", key="gen_t2_b"):
                    txt = generate_post_text(info["itemName"], "", t_str, tone, length, custom, ref, api["gemini"])
                    st.session_state["r2"] = {"text": txt}
                show_final("res2", info["itemUrl"], info["imageUrl"])

        with tab3:
            u_img = st.file_uploader("画像アップ", type=["jpg","png","webp"], key="up_t3")
            hint = st.text_input("ヒント", key="hint_t3")
            t_str, tone, length, ref, custom = draw_ui("t3")
            if st.button("✨ 解析・生成", key="gen_t3_b"):
                if u_img:
                    txt = generate_post_text(hint, "", t_str, tone, length, custom, ref, api["gemini"], image=Image.open(u_img))
                    st.session_state["r3"] = {"text": txt}
    ...
    if api["threads"]:
        with st.spinner("分析中..."):
            ...
            if raw:
                ...
                c1, c2, c3, c4 = st.columns(4)
                c1.metric("投稿数", f"{len(this_df)} 件", delta(len(this_df), len(last_df)))
                c2.metric("閲覧", f"{this_df['views'].sum():,}", delta(this_df['views'].sum(), last_df['views'].sum()))
                c3.metric("いいね", f"{this_df['likes'].sum():,}", delta(this_df['likes'].sum(), last_df['likes'].sum()))
                c4.metric("返信", f"{this_df['replies'].sum():,}", delta(this_df['replies'].sum(), last_df['replies'].sum()))
                st.divider()
...
# ------------------------------------------
    ...
    with st.expander("👤 ロード", expanded=True):
        pw = st.text_input("合言葉", type="password", key="apw_v4")
...
# ------------------------------------------
    ...
    if api["sheet_id"]:
        with st.form("tmp_v4"):
            ti = st.text_input("タイトル"); co = st.text_area("内容")
