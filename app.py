...
# ------------------------------------------
    ...
    if not api["gemini"]: st.warning("API設定を完了してください。")
        ...
        def show_final(key, default_url, default_img):
            if key in st.session_state:
                ...
                with st.expander(f"✨ 生成結果の確認・編集", expanded=True):
                    ...
                    with cs:
                        dv, tv = st.date_input("日", key=f"d_f_{key}"), st.time_input("時間", key=f"t_f_{key}")
        ...
        with tab1:
            ...
            sel_g = st.selectbox("ジャンル選択", list(genres.keys()), key="rank_final")
            if st.button("ランキング取得"):
                st.session_state["items_final"] = get_rakuten_ranking(api["rakuten_id"], api["rakuten_key"], api["rakuten_aff_id"], genres[sel_g])
            
            if "items_final" in st.session_state:
                sel_it = []
                for i, item in enumerate(st.session_state["items_final"]):
                    with st.container(border=True):
                        ...
                        if c2.checkbox(f"選ぶ: {item['itemName'][:50]}", key=f"chk_{i}"):
                            ...
                            item["user_file"] = uf; sel_it.append(item)
                
                if sel_it:
                    st.divider(); t_str, tone, length, ref, custom = draw_ui("tab1")
                    if st.button(f"✨ {len(sel_it)}件を一括生成"):
                        for it in sel_it:
...
# 🔍 分析, API設定, テンプレート管理は以前の完成版を維持
    ...
    if api["threads"]:
        ...
        if raw:
            ...
            today = datetime.now().date(); st_this = today - timedelta(days=today.weekday()); st_last = st_this - timedelta(days=7)
    ...
    with st.expander("👤 管理者ロード"):
        ...
        if st.button("Secretsからロード"):
            if pw == st.secrets.get("master_password"):
                ...
                st.success("Secretsからロードしました。保存を押してください。")
    with st.container(border=True):
        c1, c2 = st.columns(2)
