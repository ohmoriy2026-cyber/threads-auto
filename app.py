    ...
    if not api["threads"]: st.info("💡 API設定を完了してください。")
        ...
        with st.spinner("データを取得中..."):
            ...
            if raw:
                ...
                with c1: st.metric("累計投稿数", len(df)); st.bar_chart(df.groupby('timestamp').size())
...
# ------------------------------------------
    ...
    if not api["gemini"]: st.warning("API設定を完了してください。")
        ...
        def draw_ui(k):
            ...
            with c5: length = st.slider("文字数", 50, 400, 80, key=f"l_{k}")
            opts = ["手動入力"] + [t["title"] for t in templates]
            sel = st.selectbox("🧠 テンプレート呼び出し", opts, key=f"tmp_{k}")
            ref = next((t["content"] for t in templates if t["title"] == sel), "") if sel != "手動入力" else ""
            ref_post = st.text_area("🧠 参考投稿本文", value=ref, key=f"ra_{k}", height=100)
            custom = st.text_area("✍️ 自由指示", key=f"cp_{k}", height=70)
            return f"{gen}, {','.join(age)}, 子供:{kids}", tone, length, ref_post, custom

        with tab1:
            genres = {
                "🏆 総合": "0", "👗 レディース服": "100371", "👔 メンズ服": "551177", "👠 靴": "558885", 
                "👜 バッグ・ブランド": "216129", "💄 美容・コスメ": "100939", "⌚ 腕時計": "558929", 
                "💎 ジュエリー": "200162", "🍎 食品": "100227", "🍪 スイーツ": "551167", 
                "🍹 飲料": "100316", "🔌 家電": "562631", "📸 カメラ": "211742", "💻 PC": "100026", 
                "🛋 インテリア": "100804", "🛁 日用品": "215783", "🍼 キッズ・ベビー": "100533", 
                "⚽ スポーツ": "101070", "⛳ ゴルフ": "101077", "🚗 車・バイク": "503190", 
                "🧸 おもちゃ": "101164", "🎮 ゲーム": "101205", "📚 本": "200376", "💠 その他": "custom"
            }
            sel_g = st.selectbox("ランキングのジャンルを選択", list(genres.keys()), key="r_list")
            if st.button("ランキング取得", key="f_rank"):
                st.session_state["it_v8"] = get_rakuten_ranking(api["rakuten_id"], api["rakuten_key"], api["rakuten_aff_id"], genres[sel_g])
            
            if "it_v8" in st.session_state:
                sel_items = []
                for i, item in enumerate(st.session_state["it_v8"]):
                    with st.container(border=True):
                        c1, c2 = st.columns([1, 4])
                        c1.image(item["mediumImageUrls"][0]["imageUrl"])
                        c2.write(f"**{item['itemName'][:50]}**")
                        if c2.checkbox("この商品を選ぶ", key=f"chk_{i}"):
                            u_f = c2.file_uploader("📸 スクショ添付（任意：無ければ楽天画像を引用）", type=["jpg","png"], key=f"uf_{i}")
                            item["user_file"] = u_f; sel_items.append(item)
                
                if sel_items:
                    st.divider(); st.subheader("生成設定"); t_str, tone, length, ref, custom = draw_ui("t1")
                    if st.button(f"✨ {len(sel_items)}件を一括生成", key="gen_b"):
                        results = []
                        for it in sel_items:
                            ...
                            results.append({"it": it, "txt": txt, "rak_img": it["mediumImageUrls"][0]["imageUrl"]})
                        st.session_state["res_v8"] = results

            if "res_v8" in st.session_state:
                for k, res in enumerate(st.session_state["res_v8"]):
                    with st.expander(f"確認: {res['it']['itemName'][:30]}", expanded=True):
                        it = res["it"]
                        use_img = st.checkbox("🖼️ 投稿に画像を含める", value=True, key=f"img_chk_{k}")
                        drive_url = st.text_input("🔗 投稿用画像URL（空なら楽天画像）", key=f"dr_{k}")
                        mk, rk = f"m_{k}", f"r_{k}"
                        if mk not in st.session_state: st.session_state[mk] = res["txt"]
                        if rk not in st.session_state: st.session_state[rk] = f"▼ 詳細はこちら\n{it.get('affiliateUrl', it['itemUrl'])}"
                        mt = st.text_area("本文編集", key=mk, height=150); rt = st.text_area("リプライ編集", key=rk, height=80)
                        
                        f_img = None
                        if use_img: f_img = convert_drive_link(drive_url) if drive_url else res["rak_img"]
                        
                        cn, cs = st.columns(2)
                        if cn.button("🚀 即時投稿", key=f"n_{k}"):
                        ...
                        with cs:
                            dv, tv = st.date_input("日", key=f"d_{k}"), st.time_input("時", key=f"t_{k}")
                            if st.button("🗓️ 予約登録", key=f"br_{k}"):
                                row = ["", st.session_state[mk], dv.strftime('%Y/%m/%d'), str(tv.hour), str(tv.minute), "pending", "", "", st.session_state[rk], f_img]
                                if save_to_sheets(api["sheet_id"], api["g_json"], row): st.success("保存完了")

        with tab2:
            url_in = st.text_input("楽天URLを貼り付け", key="u_in_t2")
            if st.button("情報取得", key="f_url"):
                ...
                st.session_state["u_t2"] = {"name": t_m.group(1)[:50] if t_m else "商品", "img": i_m.group(1) if i_m else "", "url": url_in}
            if "u_t2" in st.session_state:
                u = st.session_state["u_t2"]; st.image(u["img"], width=150); st.write(u["name"])
                u_file_2 = st.file_uploader("📸 自分の画像を添付（任意）", type=["jpg","png"], key="uf2")
                t_str, tone, length, ref, custom = draw_ui("t2")
                if st.button("✨ 本文作成", key="gen_t2"):
                    img_in = Image.open(u_file_2) if u_file_2 else download_image(u["img"])
                    txt = generate_post_text(u["name"], "", t_str, tone, length, custom, ref, api["gemini"], image=img_in)
                    st.success("生成完了！確認画面を表示します...")
                    st.session_state["res2_v8"] = {"txt": txt, "url": u["url"], "img": u["img"]}
                if "res2_v8" in st.session_state:
                    p2 = st.session_state["res2_v8"]; m2 = st.text_area("本文", value=p2["txt"], height=150)
                    if st.button("🚀 今すぐ投稿", key="n2"):
                        if post_to_threads(api["threads"], m2, image_url=p2["img"]): st.success("成功")

        with tab3:
            u_f3 = st.file_uploader("📸 スクショアップロード", type=["jpg","png"], key="uf3")
            hint = st.text_input("商品名のヒント", key="h3")
            t_str, tone, length, ref, custom = draw_ui("t3")
            if st.button("✨ 解析して作成", key="gen_t3"):
                if u_f3:
                    txt = generate_post_text(hint, "", t_str, tone, length, custom, ref, api["gemini"], image=Image.open(u_f3))
                    st.session_state["res3_v8"] = {"txt": txt}
                else: st.error("画像が必要です")
            if "res3_v8" in st.session_state:
                st.text_area("本文", value=st.session_state["res3_v8"]["txt"], height=150)
    ...
    with st.expander("👤 管理者モード (ロード)", expanded=True):
        ...
        if st.button("設定をロード"):
            if pw == st.secrets.get("master_password"):
                ...
                st.success("Secretsからロードしました。下の「設定を保存」を押してください。")
    with st.container(border=True):
        c1, c2 = st.columns(2)
...
# ------------------------------------------
    ...
    if api["sheet_id"]:
        with st.form("tmp_f"):
            ti = st.text_input("タイトル"); co = st.text_area("内容", height=150)
