    ...
    try:
        ...
        def fetch_insights(thread):
            ...
            try:
                ...
                thread['views'], thread['like_count'], thread['reply_count'] = 0, 0, 0
            return thread
...
def get_item_info_from_url(url):
    ...
    try:
        ...
        res = requests.Session().get(url, headers=headers, timeout=10, allow_redirects=True)
        ...
        if not title: return default_info
        return {"itemName": title[:100], "imageUrl": image, "itemUrl": res.url, "itemPrice": ""}
    except Exception:
        return default_info

def create_affiliate_url(url, aff_id):
    if not aff_id: return url
    return f"https://hb.afl.rakuten.co.jp/ichiba/{aff_id}/?pc={urllib.parse.quote(url, safe='')}"
...
・添付された画像がある場合は、画像内のテキストや雰囲気も読み取り、商品の魅力として紹介文に必ず活かしてください。

【冒頭のフック指定（超重要）】
文章の最初の1行目は、必ず以下の「フックワード候補」の中から、商品のジャンルやトーンに最も適したものを1つ選び、自然な形で組み込んでスタートしてください。
...
"""
    ...
    if reference_post: prompt += f"\n\n【文体の完全模倣（超重要）】\n以下の参考投稿のテンポ、言葉選び、絵文字の使い方を完全にコピーして、今回の商品の紹介文を作成してください。\n＜参考投稿＞\n{reference_post}"
    ...
    if not api["sheet_id"] or not api["threads"]:
        ...
        if threads_data:
            ...
            if not df_main.empty:
                for i, row in df_main.sort_values(by='total_eng', ascending=False).head(5).iterrows():
                    st.markdown(f"""
                    <div class="ranking-box">
                        <div><span class="ranking-rank">#{list(df_main.sort_values(by='total_eng', ascending=False).index).index(i) + 1}</span><span style="color:#9CA3AF; font-size: 14px;">{row['timestamp']}</span></div>
...
# ==========================================
    ...
    st.title("🔍 エンゲージメント分析")
    api = st.session_state["api_keys"]
    if not api["threads"]: st.info("💡 API設定でロードを行ってください。")
    else:
        threads_data = get_threads_engagement(api["threads"])
        if threads_data:
            df = pd.DataFrame(threads_data)
            ...
            df['reply_count'] = pd.to_numeric(df['reply_count'], errors='coerce').fillna(0).astype(int)
            df['views'] = pd.to_numeric(df['views'], errors='coerce').fillna(0).astype(int)
            df['timestamp'] = pd.to_datetime(df.get('timestamp', datetime.now().isoformat())).dt.date
            
            df = df[df.get('is_reply', False) != True]
...
# ==========================================
    ...
    if not api["rakuten_id"]: st.warning("API設定を先に済ませてください。")
        ...
        with tab1:
            ...
            sel_name = st.selectbox("ジャンルを選択", list(genres_dict.keys()), key="sel_genre_p1")
            target_id = genres_dict[sel_name]
            if target_id == "custom": target_id = st.text_input("楽天ジャンルIDを入力", key="custom_id_in_p1")
            ...
            if "items" in st.session_state:
                ...
                for i, item in enumerate(st.session_state["items"]):
                    with st.container(border=True):
                        ...
                        if c2.checkbox("選ぶ", key=f"sel_chk_{i}"):
                            item["u_img"] = c2.file_uploader("📸 スクショアップロード", type=["jpg","png"], key=f"u_f_{i}")
                ...
                if selected:
                    with st.container(border=True):
                        ...
                        sel_temp = st.selectbox("🧠 テンプレート呼び出し", template_options, key="sel_temp_p1")
                        if sel_temp == "手動で入力する": ref_post = st.text_area("🧠 バズ投稿（手動入力）", key="ref_post_p1")
                        else: ref_post = st.text_area("🧠 テンプレート反映", value=next((t["content"] for t in templates if t["title"] == sel_temp), ""), key="ref_post_p1_auto")
                            
                        custom_prompt = st.text_area("✍️ 自由な追加指示", key="c_prompt_p1")
                        
                        if st.button(f"✨ {len(selected)}件の文章を生成", key="gen_btn_p1"):
                            # 新しく生成する時に過去の入力をリセット
                            for key in list(st.session_state.keys()):
                                if key.startswith("final_txt_") or key.startswith("reply_txt_"):
                            ...
                            for j, s_item in enumerate(selected):
                                ...
                                txt = generate_post_text(s_item["itemName"], s_item["itemPrice"], t_str, tone, length, custom_prompt, ref_post, api["gemini"], img_obj)
            ...
            if "gen_res_p1" in st.session_state:
                for k, p in enumerate(st.session_state["gen_res_p1"]):
                    ...
                    with st.expander(f"確認: {item['itemName'][:30]}", expanded=True):
                        # セッションステートでテキストエリアの値を管理
                        m_key = f"final_txt_{k}"
                        r_key = f"reply_txt_{k}"
                        if m_key not in st.session_state:
                            st.session_state[m_key] = p["text"]
                        if r_key not in st.session_state:
                            st.session_state[r_key] = f"▼ 詳細はこちら\n{item.get('affiliateUrl', item['itemUrl'])}"
                        
                        f_txt = st.text_area("メイン本文", key=m_key, height=150)
        ...
        # ====== タブ2: 商品URLから作る ======
        with tab2:
            input_url = st.text_input("🔗 楽天の商品URLを貼り付け", key="input_url_tab2")
            if st.button("商品情報を取得", key="get_url_btn_t2"):
                if input_url:
                    with st.spinner("取得中..."):
                        info = get_item_info_from_url(input_url)
                        if not info["itemName"]: st.warning("⚠️ 手動で入力してください！")
            ...
            if "url_item" in st.session_state:
                ...
                edit_name = c2.text_input("商品名", value=item["itemName"], key="edit_name_t2")
                edit_price = c2.text_input("価格", value=item["itemPrice"], key="edit_price_t2")
                u_img_tab2 = c2.file_uploader("📸 スクショアップロード", type=["jpg","png"], key="u_img_tab2")
                ...
                sel_temp_t2 = st.selectbox("🧠 テンプレート呼び出し", template_options, key="sel_temp_t2")
                if sel_temp_t2 == "手動で入力する": ref_post_t2 = st.text_area("🧠 バズ投稿（手動入力）", key="ref_post_t2")
                else: ref_post_t2 = st.text_area("🧠 テンプレート反映", value=next((t["content"] for t in templates if t["title"] == sel_temp_t2), ""), key="ref_post_t2_auto")
                    
                cp_t2 = st.text_area("✍️ 自由な追加指示", key="cp_t2")
                
                if st.button("✨ 生成", key="gen_btn_tab2"):
                    # 新しく生成する時に過去の入力をリセット
                    if "final_txt_t2" in st.session_state: del st.session_state["final_txt_t2"]
                    if "reply_txt_t2" in st.session_state: del st.session_state["reply_txt_t2"]

                    if not edit_name and not u_img_tab2: st.error("商品名か画像が必須です")
                    else:
                        ...
                        with st.spinner("執筆中..."):
                            ...
                            st.session_state["gen_res_tab2"] = {"text": txt, "target_url": create_affiliate_url(item["itemUrl"], api["rakuten_aff_id"]), "imageUrl": item["imageUrl"]}

            if "gen_res_tab2" in st.session_state:
                p = st.session_state["gen_res_tab2"]
                
                # セッションステートでテキストエリアの値を管理
                if "final_txt_t2" not in st.session_state:
                    st.session_state["final_txt_t2"] = p["text"]
                if "reply_txt_t2" not in st.session_state:
                    st.session_state["reply_txt_t2"] = f"▼ 詳細はこちら\n{p['target_url']}"

                f_txt_t2 = st.text_area("メイン本文", key="final_txt_t2", height=150)
                reply_txt_t2 = st.text_area("リプライ文章", key="reply_txt_t2", height=100)
                ...
                with c_sch_t2:
                    ...
                    if st.button("🗓️ 予約リストに追加", key="reserve_final_btn_t2"):
                        ...
                        if save_to_sheets(api["sheet_id"], api["g_json"], row): st.success(f"✅ 保存しました！")
...
# ==========================================
    ...
    if not api["sheet_id"] or not api["g_json"]:
        ...
        with st.container(border=True):
            ...
            new_title = st.text_input("テンプレート名")
            new_content = st.text_area("バズ投稿の本文", height=150)
            if st.button("💾 保存", key="save_temp_btn"):
        ...
        if templates:
            for i, t in enumerate(templates):
                with st.expander(f"📌 {t['title']}"): st.write(t["content"])
        else: st.info("テンプレートがありません。")
...
# ==========================================
    ...
    with st.expander("👤 管理者モード", expanded=True):
        ...
        if st.button("ロード", key="load_btn"):
            if pw == st.secrets.get("master_password"):
                ...
                st.success("✅ ロード成功！「設定を保存」を押してください。")
    ...
    with st.container(border=True):
        ...
        r_aff = c1.text_input("楽天 アフィリエイトID", type="password", key="f_ra")
