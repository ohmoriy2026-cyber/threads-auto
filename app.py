...
if page == "1. ダッシュボード":
    ...
    if not api["sheet_id"] or not api["threads"]:
        ...
        if threads_data:
            ...
            with c1: st.metric("📝 累計投稿数", f"{len(df_main)} 件"); st.bar_chart(df_main.groupby('date_key').size())
            with c2: st.metric("❤️ 累計いいね数", f"{df_main['like_count'].sum():,} 回"); st.bar_chart(df_main.groupby('date_key')['like_count'].sum(), color="#FF4B4B")
            with c3: st.metric("💬 累計リプライ", f"{df_main['reply_count'].sum():,} 件"); st.bar_chart(df_main.groupby('date_key')['reply_count'].sum(), color="#FFB800")
...
# --- 2. 商品作成＆予約 ---
    ...
    if not api["rakuten_id"] or not api["gemini"]: 
        ...
        def draw_ui(k):
            ...
            c4, c5 = st.columns(2)
            with c4: tone = st.selectbox("トーン", ["エモい", "役立つ", "元気", "親近感", "本音レビュー", "あざと可愛い", "高級感", "ズボラ命"], key=f"s_tone_{k}")
        ...
        def show_final_ui(key, default_txt, default_url, default_img_url):
            with st.expander("✨ 生成結果の確認・編集", expanded=True):
                ...
                drive_url = st.text_input("🔗 投稿用画像URL (空欄なら初期画像を引用)", value=default_img_url if default_img_url else "", key=f"drive_{key}")
                ...
                final_img = convert_drive_link(drive_url) if use_img and drive_url else (default_img_url if use_img else None)
                ...
                with c_sch:
                    d = st.date_input("予約日", key=f"d_in_{key}"); t = st.time_input("時間", key=f"t_in_{key}")
                    if st.button("🗓️ 予約リストに追加", key=f"reserve_{key}"):
                        ...
                        if save_to_sheets(api["sheet_id"], api["g_json"], row): st.success("✅ 保存しました！")
        ...
        with tab1:
            ...
            if "items_t1" in st.session_state:
                ...
                for i, item in enumerate(st.session_state["items_t1"]):
                    with st.container(border=True):
                        ...
                        if c2.checkbox(f"選ぶ: {item['itemName'][:50]}", key=f"chk_t1_{i}"):
                            item["u_img"] = c2.file_uploader("📸 スクショ添付", type=["jpg","png"], key=f"uf_t1_{i}")
            ...
            if "res_list_t1" in st.session_state:
                for k, p in enumerate(st.session_state["res_list_t1"]):
                    aff_url = create_affiliate_link(p["item"].get("itemUrl", ""), str(api["rakuten_aff_id"]).strip())
                    show_final_ui(f"r1_{p['item']['itemCode']}", p["text"], aff_url, p["item"]["mediumImageUrls"][0]["imageUrl"])
        ...
        with tab2:
            ...
            if "item_t2" in st.session_state:
                it = st.session_state["item_t2"]; st.image(it["img"], width=150); t_str, tone, length, ref, cp = draw_ui("t2")
                ...
                if "res_t2" in st.session_state:
                    aff_url_t2 = create_affiliate_link(it["url"], str(api["rakuten_aff_id"]).strip())
                    show_final_ui("res_t2_final", st.session_state["res_t2"]["text"], aff_url_t2, it["img"])
        ...
        with tab3:
            ...
            if "res_t3" in st.session_state:
                uf_t3 = st.file_uploader("📸 【追加】投稿用スクショ添付", type=["jpg","png"], key="uf_t3")
                aff_t3 = st.text_input("🔗 アフィリエイト商品URL", key="aff_url_t3")
...
# --- 4. API設定 (方法1 & 方法2 統合) ---
    ...
    if stored and "rakuten_id" in stored and not st.session_state.get("f_ri"):
        for k, v in stored.items(): st.session_state[f"f_{k}"] = v
    ...
    with st.expander("👤 管理者モード (Secretsロード)", expanded=True):
        ...
        if st.button("一括ロード"):
            if pw == st.secrets.get("master_password"):
                for k, v in {"ri":"rakuten_id","rk":"rakuten_key","ra":"rakuten_aff_id","gk":"gemini_key","tt":"threads_token","si":"sheet_id","gj":"g_json"}.items():
                    st.session_state[f"f_{k}"] = st.secrets.get(v, "")
                st.success("✅ ロード成功！")
    ...
    with st.container(border=True):
        ...
        if st.button("✅ 設定をブラウザに記憶して保存"):
            ...
            # 👇 方法2: ブラウザ（Local Storage）に保存
            local_storage.setItem("threads_marketing_keys", data)
            st.success("🎉 ブラウザに記憶しました！次回から入力不要です。")
...
# --- 5. テンプレート管理 ---
    ...
    if not api["sheet_id"]: st.warning("API設定を完了してください。")
        ...
        with st.form("tf"):
            ...
            t_content = st.text_area("バズ投稿の本文", height=150)
