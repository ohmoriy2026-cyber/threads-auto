    ...
    if not api["threads"]: st.info("💡 API設定を完了してください。")
        ...
        with st.spinner("集計中..."):
            ...
            if raw:
                ...
                t_list = [r for r in data if r.get('投稿日') == today]
                if t_list: st.dataframe(pd.DataFrame(today_list)[['時', '分', '本文']], use_container_width=True)
    ...
    if not api["gemini"]: st.warning("API設定を完了してください。")
        ...
        with tab1:
            genres = {
                ...
                "👜 バッグ": "216129", "💄 美容": "100939", "⌚ 腕時計": "558929", "💎 ジュエリー": "200162", 
                "🍎 食品": "100227", "🍪 スイーツ": "551167", "🔌 家電": "562631", "🍼 ベビー": "100533", 
                "⚽ スポーツ": "101070", "🚗 車・バイク": "503190", "🎮 ゲーム": "101205", "📚 本": "200376"
            }
            sel_g = st.selectbox("ランキングのジャンルを選択", list(genres.keys()), key="rank_v10")
            if st.button("ランキング取得", key="btn_rank_v10"):
                st.session_state["items_v10"] = get_rakuten_ranking(api["rakuten_id"], api["rakuten_key"], api["rakuten_aff_id"], genres[sel_g])
            
            if "items_v10" in st.session_state:
                sel_items = []
                for i, item in enumerate(st.session_state["items_v10"]):
                    with st.container(border=True):
                        c1, c2 = st.columns([1, 4])
                        c1.image(item["mediumImageUrls"][0]["imageUrl"])
                        if c2.checkbox(f"選ぶ: {item['itemName'][:50]}", key=f"chk_v10_{i}"):
                            uf = c2.file_uploader("📸 スクショ添付（任意）", type=["jpg","png"], key=f"uf_v10_{i}")
                            item["user_file"] = uf; sel_items.append(item)
                
                if sel_items:
                    st.divider(); st.subheader("生成設定"); t_str, tone, length, ref, custom = draw_ui("tab1")
                    if st.button(f"✨ {len(sel_items)}件を一括生成", key="gen_v10"):
                        results = []
                        for it in sel_items:
                            ...
                            results.append({"it": it, "txt": txt, "img": it["mediumImageUrls"][0]["imageUrl"]})
                        st.session_state["res_v10"] = results

            if "res_v10" in st.session_state:
                for k, res in enumerate(st.session_state["res_v10"]):
                    with st.expander(f"確認: {res['it']['itemName'][:30]}", expanded=True):
                        use_img = st.checkbox("🖼️ 投稿に画像を含める", value=True, key=f"ui_v10_{k}")
                        drive_url = st.text_input("🔗 投稿用画像URL (空なら楽天画像)", key=f"dr_v10_{k}")
                        
                        mk, rk = f"m_v10_{k}", f"r_v10_{k}"
                        if mk not in st.session_state: st.session_state[mk] = res["txt"]
                        if rk not in st.session_state: st.session_state[rk] = f"▼ 詳細はこちら\n{res['it'].get('affiliateUrl', res['it']['itemUrl'])}"
                        mt = st.text_area("本文編集", key=mk, height=150); rt = st.text_area("リプライ編集", key=rk, height=80)
                        
                        f_img = None
                        if use_img: f_img = convert_drive_link(drive_url) if drive_url else res["img"]
                        
                        cn, cs = st.columns(2)
                        if cn.button("🚀 即時投稿", key=f"now_v10_{k}"):
                        ...
                        with cs:
                            dv, tv = st.date_input("予約日", key=f"d_v10_{k}"), st.time_input("時間", key=f"t_v10_{k}")
                            if st.button("🗓️ 予約登録", key=f"br_v10_{k}"):
        ...
        # Tab2, Tab3も同様の設定（省略せず完備）
        with tab2:
            url_in = st.text_input("楽天URL貼り付け", key="u_in_t2")
            if st.button("取得", key="bv2"):
            ...
            if "u_t2" in st.session_state:
                u = st.session_state["u_t2"]; st.image(u["img"], width=150); t_str, tone, length, ref, custom = draw_ui("t2")
                if st.button("✨ 本文作成", key="gen_t2"):
                    st.session_state["res2_v10"] = {"txt": generate_post_text(u["name"], "", t_str, tone, length, custom, ref, api["gemini"], image=download_image(u["img"])), "url": u["url"], "img": u["img"]}
                if "res2_v10" in st.session_state:
                    p2 = st.session_state["res2_v10"]; m2 = st.text_area("本文", value=p2["txt"], key="m2_v10", height=150)
                    if st.button("🚀 今すぐ投稿", key="n2_v10"):
                        if post_to_threads(api["threads"], m2, image_url=p2["img"]): st.success("成功")

        with tab3:
            uf3 = st.file_uploader("📸 スクショアップ", type=["jpg","png"], key="uf3_v10")
            hint = st.text_input("ヒント", key="h3_v10"); t_str, tone, length, ref, custom = draw_ui("t3")
            if st.button("✨ 解析して作成", key="gen_t3_v10"):
                if uf3:
                    txt = generate_post_text(hint, "", t_str, tone, length, custom, ref, api["gemini"], image=Image.open(uf3))
                    st.session_state["res3_v10"] = {"txt": txt}
                else: st.error("画像が必要です")
            if "res3_v10" in st.session_state:
                st.text_area("本文", value=st.session_state["res3_v10"]["txt"], height=150)
    ...
    if api["threads"]:
        with st.spinner("集計中..."):
            ...
            if raw:
                ...
                this_df = df[df['timestamp'] >= st_this]; last_df = df[(df['timestamp'] >= start_last) if 'start_last' in locals() else df['timestamp'] < st_this]
...
# ------------------------------------------
    ...
    st.title("⚙️ API設定")
    with st.expander("👤 管理者ロード"):
        pw = st.text_input("合言葉", type="password", key="apw_v10")
