...
def generate_post_text(item_name, price, target_str, tone, length, custom_prompt, reference_post, api_key, image=None):
    ...
    prompt = f"""あなたは、SNSでリアルな本音を発信するインフルエンサーです。楽天商品「{item_name}」{price_str}を、ターゲット【{target_str}】に向けて、{tone}なテイストで約{length}文字で紹介してください。
【絶対厳守】
...
# ブラウザ保存からの自動復旧（自分のPCのブラウザにだけ保存されている値を読む）
if not st.session_state["api_keys"]["rakuten_id"]:
    stored = local_storage.getItem("threads_marketing_keys")
    if stored:
        st.session_state["api_keys"].update({
            "rakuten_id": stored.get("rakuten_id", ""),
            "rakuten_key": stored.get("rakuten_key", ""),
            "rakuten_aff_id": stored.get("rakuten_aff_id", ""),
            "gemini": stored.get("gemini_key", ""),
            "threads": stored.get("threads_token", ""),
            "sheet_id": stored.get("sheet_id", ""),
            "g_json": stored.get("g_json", "")
        })
...
if page == "1. ダッシュボード":
    ...
    if not api["rakuten_id"] or not api["threads"]:
        st.warning("⚠️ API設定が完了していません。「4. API設定」で設定を行ってください。")
    else:
...
# --- 2. 商品作成＆予約 ---
    ...
    if not api["rakuten_id"] or not api["gemini"]: 
        ...
        with tab3:
            ...
            img_url_t3 = st.text_input("🔗 画像URLを入力 (Googleドライブ等)", key="u3")
            ...
            if "res3" in st.session_state:
                ...
                show_final_ui("res_t3_final", st.session_state["res3"]["text"], final_reply_url, st.session_state["res3"]["url"])
...
# --- 4. API設定 (同期問題を解決) ---
    ...
    with st.expander("👤 管理者モード (Secretsからロード)", expanded=True):
        st.info("この機能は、サイト管理者が自身のSecrets情報を読み込むためのものです。他人がここを使っても何も起きません。")
        pw = st.text_input("合言葉", type="password", key="master_pw_input")
        if st.button("一括ロードしてシステムに反映"):
            if pw == st.secrets.get("master_password"):
                st.session_state["api_keys"].update({
                    "rakuten_id": st.secrets.get("rakuten_id", ""),
                    "rakuten_key": st.secrets.get("rakuten_key", ""),
                    "rakuten_aff_id": st.secrets.get("rakuten_aff_id", ""),
                    "gemini": st.secrets.get("gemini_key", ""),
                    "threads": st.secrets.get("threads_token", ""),
                    "sheet_id": st.secrets.get("sheet_id", ""),
                    "g_json": st.secrets.get("g_json", "")
                })
                st.success("✅ 管理者設定をロードしました！")
                st.rerun()
            else: st.error("❌ 合言葉が違います")

    with st.container(border=True):
        st.subheader("💡 個別設定（自分のPCのブラウザにだけ保存）")
        st.write("ここに入力して下のボタンを押すと、**あなたのブラウザにだけ**設定が保存されます。他の人には見えません。")
        c1, c2 = st.columns(2)
        r_id = c1.text_input("楽天 App ID", value=api["rakuten_id"], type="password")
        r_key = c1.text_input("楽天 Access Key", value=api["rakuten_key"], type="password")
        r_aff = c1.text_input("楽天 アフィリエイトID", value=api["rakuten_aff_id"], type="password")
        g_key = c2.text_input("Gemini API", value=api["gemini"], type="password")
        t_tok = c2.text_input("Threads Token", value=api["threads"], type="password")
        s_id = c2.text_input("Sheet ID", value=api["sheet_id"])
        g_js = st.text_area("GCloud JSON", value=api["g_json"], height=100)
        
        if st.button("✅ 設定をこのPCのブラウザに保存する"):
            ...
            # 💡 ブラウザの個別ストレージに保存（他人と共有されません）
            local_storage.setItem("threads_marketing_keys", data)
            st.success("🎉 このブラウザに設定を記憶しました！次回から入力不要です。")
            st.rerun()
