    ...
    if not api["rakuten_id"]: 
            ...
            if "items" in st.session_state:
                ...
                if selected:
                    ...
                    with st.container(border=True):
                        ...
                        # 🌟 学習用バズ投稿入力欄の追加
                        ref_post_p1 = st.text_area("🧠 参考にするバズ投稿（文体・テンポの模倣用）", placeholder="あのバズっている投稿の文章をコピペしてください", key="ref_post_p1")
                    ...
                    with st.expander(f"確認: {item['itemName'][:30]}", expanded=True):
                        ...
                        with c_sch:
                            ...
                            if st.button("🗓️ 予約リストに追加", key=f"reserve_final_btn_{k}"):
                                # 予約時にもユーザーが編集したリプライ文章を保存（GASで読み取ります）
        ...
        with tab2:
            ...
            if "url_item" in st.session_state:
                ...
                # 🌟 学習用バズ投稿入力欄の追加
                ref_post_t2 = st.text_area("🧠 参考にするバズ投稿（文体・テンポの模倣用）", placeholder="あのバズっている投稿の文章をコピペしてください", key="ref_post_t2")
