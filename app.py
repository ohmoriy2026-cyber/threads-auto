...
if page == "1. ダッシュボード":
    ...
    if not api["rakuten_id"] or not api["threads"]:
        st.warning("⚠️ API設定が完了していません。「4. API設定」でロードまたは入力を行ってください。")
    else:
...
# --- 4. API設定 (同期問題を解決) ---
    ...
    with st.expander("👤 管理者モード (Secretsロード)", expanded=True):
        ...
        if st.button("一括ロードしてシステムに反映"):
            if pw == st.secrets.get("master_password"):
                ...
                st.success("✅ システムへの反映が完了しました！商品画面等が使えます。")
                st.rerun() # 画面を更新して即時反映させる
    ...
    with st.container(border=True):
        ...
        if st.button("✅ 設定をブラウザに保存 ＆ システムに反映"):
            data = {"rakuten_id":r_id, "rakuten_key":r_key, "rakuten_aff_id":r_aff, "gemini":g_key, "threads":t_tok, "sheet_id":s_id, "g_json":g_js}
            # アプリ全体で使う変数へ反映
            st.session_state["api_keys"].update(data)
            # ブラウザのストレージへ保存
            local_storage.setItem("threads_marketing_keys", {
                "rakuten_id":r_id, "rakuten_key":r_key, "rakuten_aff_id":r_aff, 
                "gemini_key":g_key, "threads_token":t_tok, "sheet_id":s_id, "g_json":g_js
            })
            st.success("🎉 システムに反映し、ブラウザにも記憶しました！")
            st.rerun()
